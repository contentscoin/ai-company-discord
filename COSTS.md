...
| 프로필 5개의 **대화·회의·오케스트레이션** | LLM API 키 **또는 구독 OAuth** — hermes 0.19.0이 `anthropic`(Claude 구독)·`openai-codex`(ChatGPT 구독)·`xai-oauth` 등 OAuth 로그인을 네이티브 지원 (실측, [ALIGNMENT.md](./ALIGNMENT.md) §1 정정). `hermes auth add <provider> --type oauth` | Hermes 게이트웨이 |
